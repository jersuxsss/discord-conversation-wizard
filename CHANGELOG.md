...
### Added
- **Date Validator** - New `date` validator in `validators.ts` to validate date inputs.
- **Session Store** - Added `SessionStore` interface and support for external session persistence in `Wizard.ts`.

### Changed
- **Strict Typing** - Replaced `any` with `unknown` and specific types in `types.ts`, `Wizard.ts`, and `validators.ts` for better type safety.
- **Improved Validators** - Updated existing validators to use `unknown` input type.
